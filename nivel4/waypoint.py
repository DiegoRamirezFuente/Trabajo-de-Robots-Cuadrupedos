...
# === Variables globales ===
waypoints = [(1.2, 0), (2.5, 0), (3, 1), (6.15, 1)]
...
def odom_callback(msg):
    ...
    # rospy.loginfo(f"Pitch actual: {pitch:.3f} rad")  # Eliminado para no imprimir constantemente

# === Movimiento hacia el waypoint ===
def move_to_waypoint(waypoint):
    global current_x, current_y, current_theta, in_hole, current_pitch
    ...
    pitch_threshold_hole = 0.02      # Agujero
    pitch_threshold_critical = 0.05  # Peligro de vuelco
    ...
    if current_waypoint_index >= 1:
        ...
        if abs(current_pitch) > pitch_threshold_hole:
            if not in_hole:
                rospy.loginfo("Inclinación detectada. Reduciendo velocidad.")
            in_hole = True
        else:
            if in_hole:
                rospy.loginfo("Terreno nivelado. Restaurando velocidad.")
            in_hole = False
    else:
        in_hole = False
